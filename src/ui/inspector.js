...
export function selectNode(id) {
    ...
    Object.keys(node.params).forEach(k => {
        ...
        if(p.type === 'file') {
            ...
            // Shortened for brevity, keep existing file logic
            ...
            // Adjust accept type based on node category or specific param
            ...
            // Trigger Button
            inputHtml = `<button class="w-full bg-red-600 text-white font-bold py-2 rounded active:bg-red-800" id="btn-trig-${id}-${k}">FIRE</button>`;
            row.innerHTML = inputHtml;
            pnl.appendChild(row);
            
            const btn = row.querySelector(`#btn-trig-${id}-${k}`);
            btn.onmousedown = () => node.params[k].v = 1;
            // Reset happens in logic loop next frame
        } else {
            ...
            const onMove = (e) => {
                ...
                // Sensitivity: Shift = 0.01, Default = 0.1, Ctrl = 1.0
                let mult = 0.01;
                ...
                if(p.step && Number.isInteger(1/p.step)) { 
                    // Integers mainly
            ...
            valBox.ondblclick = () => {
                ...
                function finishEdit() {
                    ...
                    if(isNaN(v)) v = node.params[k].v;
                    node.params[k].v = v;
                    valBox.innerText = v.toFixed(3);
                    // Re-enable drag? It's just replaced by text
